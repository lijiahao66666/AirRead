...
AirRead 是一个本地优先的沉浸式双语阅读器。用户可以在浏览器中导入自己的 EPUB、TXT、Markdown、HTML 或文字型 PDF，阅读时生成本章双语，也可以选中片段按需翻译或朗读；书籍工作室负责处理和导出电子书。

## 产品能力

- **书架与阅读器**：支持 EPUB 2/3、UTF-8/GBK/GB18030 TXT、Markdown、HTML 和文字型 PDF；它们都可进入章节目录、进度记忆、本章双语、划词翻译和设备语音朗读流程。扫描版 PDF 需先在其他工具中完成 OCR。
...
- **本地优先**：书籍、进度、翻译缓存、阅读排版、朗读音色与速度、书籍翻译偏好、划词翻译偏好和翻译服务配置保存在当前浏览器，打开网页即可使用。
...
- 书籍原始文件、解析后的章节、封面、阅读进度、书籍正文目标语言和划词目标语言保存在浏览器 IndexedDB；阅读排版和朗读偏好保存在当前浏览器。
