...
- **免费 + 自带密钥**：内置免费翻译，默认自动按 MyMemory → Azure Edge → Google 尝试；也可以在设置中选择单条免费线路。自定义服务分为“大语言模型翻译”（OpenAI 兼容协议）和“专用翻译 API”（腾讯云翻译 TMT、Azure AI Translator、有道智云文本翻译、DeepL API）。
...
免费翻译服务的可用性和配额由第三方决定。自动线路会按 MyMemory、Azure Edge、Google 顺序尝试；Google 线路需要可访问 Google 的网络环境，Azure Edge 是微软 Edge 使用的非官方无 Key 接口，可能随时变化。重要书籍建议配置自己的翻译服务，并在导出前抽样校对译文。
