...
String _decodeObfuscated(String enc) {
  ...
  const salt = String.fromEnvironment('AIRREAD_TENCENT_XOR_SALT', defaultValue: '');
...
TencentCredentials getEmbeddedPublicHunyuanCredentials() {
  const appIdEnv = String.fromEnvironment('AIRREAD_TENCENT_APP_ID', defaultValue: '');
  const secretIdEnv =
  ...
  final appId = _decodeObfuscated(appIdEnv.trim().isNotEmpty ? appIdEnv : _publicAppIdEnc);
  final secretId =
      _decodeObfuscated(secretIdEnv.trim().isNotEmpty ? secretIdEnv : _publicSecretIdEnc);
