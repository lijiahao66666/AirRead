...
class _IllustrationPanelState extends State<IllustrationPanel> {
  ...
  Future<void> _reanalyzeChapter({
    ...
    if (aiModel.source == AiModelSource.local) {
      if (!aiModel.loaded || !aiModel.localImageReady) {
        _showToast('本地模型未就绪');
        return;
      }
    ...
    try {
      ...
      await provider.analyzeChapter(
        chapterId: widget.chapterId,
        chapterTitle: widget.chapterTitle.trim().isEmpty ? '正文' : widget.chapterTitle.trim(),
        content: content,
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<IllustrationProvider>(
      builder: (context, provider, child) {
        ...
        final analyzingOrQueued = provider.isAnalyzingOrQueued(widget.chapterId);
                  ...
                  Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (widget.onlySceneId == null) const SizedBox(height: 8),
                        _settingsRow(
                          label: '风格',
                          child: Wrap(
                            spacing: 8,
