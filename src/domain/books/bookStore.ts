...
export function createBookStore(open: OpenBookDatabase = openBookDatabase) {
  ...
  return {
    ...
    async updateBook(id: string, updates: Partial<Pick<Book, 'lastReadAt' | 'readingChapter' | 'readingProgress' | 'readingAnchor' | 'bookmarks' | 'excerpts' | 'generatedBilingual' | 'translationPreferences' | 'selectionPreferences'>>): Promise<void> {
