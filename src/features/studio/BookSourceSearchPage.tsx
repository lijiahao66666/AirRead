...
export function BookSourceSearchPage({ onBack, search = searchPublicBookSources }: BookSourceSearchPageProps) {
  ...
  return <section className="studio-page" aria-labelledby="book-source-search-title">
    ...
    <header className="studio-page__header studio-page__header--tool"><div><p className="eyebrow">书籍工作室 · 书籍搜索</p><h2 id="book-source-search-title">搜索书籍</h2><p className="page-lede">输入书名、作者或关键词，快速检索当前可访问的中文书目。</p></div><BookOpen size={40} strokeWidth={1.35} aria-hidden="true" /></header>
    <section className="studio-card source-search" aria-label="书籍搜索">
      <div className="studio-card__heading"><div><p className="eyebrow">统一书目检索</p><h3>搜索书目</h3></div><span>快速返回</span></div>
      <form className="source-search__form" onSubmit={(event) => { void submit(event); }}>
        ...
        <button className="primary-action" type="submit" disabled={searching}>{searching ? <LoaderCircle className="is-spinning" size={17} /> : <Search size={17} />} 搜索书目</button>
      </form>
