...
export function BookSourceSearchPage({ onBack, search = searchPublicBookSources }: BookSourceSearchPageProps) {
  ...
  return <section className="studio-page" aria-labelledby="book-source-search-title">
    ...
    <header className="studio-page__header studio-page__header--tool"><div><p className="eyebrow">书籍工作室 · 书籍搜索</p><h2 id="book-source-search-title">搜索书籍</h2><p className="page-lede">输入一次关键词，AirRead 会汇总当前可访问的中文典籍索引和公共领域书目。</p></div><BookOpen size={40} strokeWidth={1.35} aria-hidden="true" /></header>
    <section className="studio-card source-search" aria-label="书籍搜索">
      ...
      <p className="source-search__notice">中文典籍仅提供站内检索与来源跳转；公共领域书目会标注来源。内容使用与授权以来源页面为准。</p>
