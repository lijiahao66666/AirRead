...
export type BookSourceSearchPageProps = {
  ...
  createBook?: (page: WikisourcePage) => Book;
};

export function BookSourceSearchPage({ onBack, onImportBook, search = searchWikisource, loadPage = loadWikisourcePage, createBook = createWikisourceBook }: BookSourceSearchPageProps) {
  ...
  const importAndRead = async (result: WikisourceSearchResult) => {
    if (importingTitle) return;
    setImportingTitle(result.title);
    setError(undefined);
    try {
      const page = await loadPage(result.title);
      await onImportBook(createBook(page));
    } catch (cause) {
  ...
  return <section className="studio-page" aria-labelledby="book-source-search-title">
    ...
    <header className="studio-page__header studio-page__header--tool"><div><p className="eyebrow">书籍工作室 · 开放作品</p><h2 id="book-source-search-title">搜索并导入作品</h2><p className="page-lede">检索中文维基文库的开放全文，导入后立即进入阅读。</p></div><BookOpen size={40} strokeWidth={1.35} aria-hidden="true" /></header>
    <section className="studio-card source-search" aria-label="开放作品搜索">
      <div className="studio-card__heading"><div><p className="eyebrow">中文维基文库 · 开放全文</p><h3>搜索作品</h3></div><span>可直接阅读</span></div>
      <form className="source-search__form" onSubmit={(event) => { void submit(event); }}>
        <label><span className="sr-only">搜索开放作品</span><Search size={18} aria-hidden="true" /><input value={query} onChange={(event) => { setQuery(event.target.value); setHasSearched(false); }} placeholder="书名、作者或关键词" aria-label="搜索开放作品" /></label>
      ...
      <p className="source-search__notice">仅显示可读取正文的开放作品。导入内容保存在当前设备，授权说明随书籍来源一同保存。</p>
      ...
      {results.length > 0 && <div className="source-search__results">{results.map((result) => <article key={result.title}>
        <div><span className="source-search__provider">中文维基文库 · 开放全文</span><h4>{result.title}</h4><p>{result.snippet || '可读取正文'}</p><small>{result.wordCount > 0 ? `约 ${result.wordCount.toLocaleString()} 字` : '篇幅以页面正文为准'}</small></div>
