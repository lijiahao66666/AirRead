...
describe('Project Gutenberg client', () => {
  ...
  it('downloads a trusted EPUB route and records its source', async () => {
    ...
    expect(fetcher).toHaveBeenCalledWith('/api/open-books/gutenberg/1342.epub', { headers: { Range: 'bytes=0-65535' } });
  ...
  it('retries incomplete range responses and joins the EPUB bytes', async () => {
    ...
    expect(fetcher).toHaveBeenNthCalledWith(1, '/api/open-books/gutenberg/1.epub', { headers: { Range: 'bytes=0-65535' } });
    expect(fetcher).toHaveBeenNthCalledWith(2, '/api/open-books/gutenberg/1.epub', { headers: { Range: 'bytes=0-65535' } });
