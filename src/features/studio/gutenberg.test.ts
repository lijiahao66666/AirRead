...
describe('Project Gutenberg client', () => {
  ...
  it('downloads a trusted EPUB route and records its source', async () => {
    ...
    expect(fetcher).toHaveBeenCalledWith('/api/open-books/gutenberg/file/1342/pg1342.epub', expect.objectContaining({ headers: { Range: 'bytes=0-65535' } }));
  ...
  it('retries incomplete range responses and joins the EPUB bytes', async () => {
    ...
    expect(fetcher).toHaveBeenNthCalledWith(1, '/api/open-books/gutenberg/file/1/pg1.epub', expect.objectContaining({ headers: { Range: 'bytes=0-65535' } }));
    expect(fetcher).toHaveBeenNthCalledWith(2, '/api/open-books/gutenberg/file/1/pg1.epub', expect.objectContaining({ headers: { Range: 'bytes=2-3' } }));
  });
});
