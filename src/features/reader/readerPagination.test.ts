...
describe('reader pagination', () => {
  it('splits long paragraphs into navigable pages without losing text', () => {
    ...
    const pages = paginateReaderParagraphs([{ id: 'p1', original }], { blockCapacity: 120, showTranslations: false });
  ...
  it('keeps original and translation chunks together in bilingual mode', () => {
    const pages = paginateReaderParagraphs([{ id: 'p1', original: 'This is a long paragraph. '.repeat(12), translation: '这是一段很长的译文。'.repeat(12) }], { blockCapacity: 160, showTranslations: true });
