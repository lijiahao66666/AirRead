...
import { Check, Copy, Languages, Play, RotateCcw, X } from 'lucide-react';
...
export function SelectionActions({ source, targetLanguage, globalTargetLanguage, targetOverride, anchor, translation, loading, error, notice, copied, canRead, onTranslate, onRead, onTargetLanguageChange, onCopy, onDismiss }: SelectionActionsProps) {
  ...
  return <aside className={`selection-actions ${expanded ? 'selection-actions--expanded' : ''}`} style={style} aria-label="选中文本操作">
    {expanded ? <>
      <div className="selection-actions__heading"><span><Languages size={15} /> 划词翻译</span><button type="button" onClick={onDismiss} aria-label="关闭划词翻译"><X size={15} /></button></div>
...
function TargetLanguageSelect({ targetLanguage, globalTargetLanguage, targetOverride, onChange, compact = false, disabled = false }: TargetLanguageSelectProps) {
  return <label className={`selection-actions__target ${compact ? 'selection-actions__target--compact' : ''}`}><span>译为</span><select aria-label="划词翻译目标语言" value={targetOverride ?? ''} onChange={(event) => onChange(event.target.value as '' | Exclude<ReaderLanguage, 'auto'>)} disabled={disabled}><option value="">跟随全局（{languageLabel(globalTargetLanguage)}）</option>{READER_LANGUAGE_OPTIONS.filter((option) => option.value !== 'auto').map((option) => <option value={option.value} key={option.value}>{option.label}</option>)}</select></label>;
}
