...
import { BookOpen, ChevronRight, Languages, List, Moon, PanelBottom, Search, Settings2, Sun, TextSelect, Volume2, X } from 'lucide-react';
...
import { phraseSelectionPointKey, phraseTextSegments, resolvePhraseSelection, type PhraseSelectionPoint, type PhraseSelectionRange } from './phraseSelection';
...
import { createReaderPageBlocks, paginateMeasuredReaderBlocks, paginateReaderParagraphs, type ReaderContentMode, type ReaderPage, type ReaderPageBlock } from './readerPagination';
import './reader.css';
...
const pageForReadingAnchor = (pages: ReaderPage[], paragraphId: string, sourceOffset: number): number => {
  const pageIndex = pages.findIndex((page) => page.some((block) => block.paragraphId === paragraphId && block.sourceStart <= sourceOffset && block.sourceEnd > sourceOffset));
  if (pageIndex >= 0) return pageIndex;
  return pages.findIndex((page) => page.some((block) => block.paragraphId === paragraphId && block.sourceStart >= sourceOffset));
};
const readingAnchorForPage = (page: ReaderPage, chapter: number): Book['readingAnchor'] => {
  const firstBlock = page[0];
  return firstBlock ? { chapter, paragraphId: firstBlock.paragraphId, sourceOffset: firstBlock.sourceStart } : undefined;
};
const bookmarkIdForAnchor = (anchor: BookReadingAnchor): string => `${anchor.chapter}:${anchor.paragraphId}:${anchor.sourceOffset}`;
const searchExcerpt = (text: string, matchIndex: number, queryLength: number): string => {
  const start = Math.max(0, matchIndex - 38);
  const end = Math.min(text.length, matchIndex + queryLength + 78);
  return `${start > 0 ? '…' : ''}${text.slice(start, end).replace(/\s+/gu, ' ')}${end < text.length ? '…' : ''}`;
};
const samePageLayout = (left: ReaderPage[], right: ReaderPage[]): boolean => left.length === right.length && left.every((page, index) => page.length === right[index]?.length && page.every((block, blockIndex) => block.id === right[index][blockIndex]?.id));
...
export function ReaderPage({ book, chapters, engine, onProgress, onTranslationPreferencesChange, onSelectionPreferencesChange, onBookmarksChange, onExcerptsChange, onBack }: ReaderPageProps) {
  ...
  const [phraseSelectionActive, setPhraseSelectionActive] = useState(false);
  const [phraseSelectionStart, setPhraseSelectionStart] = useState<PhraseSelectionPoint>();
  const [phraseSelectionRange, setPhraseSelectionRange] = useState<PhraseSelectionRange>();
  ...
  const readerPageMeasureRef = useRef<HTMLDivElement>(null);
  ...
  const [measuredPagination, setMeasuredPagination] = useState<{ key: string; pages: ReaderPage[] }>();
  ...
  const resetTransientState = () => {
    ...
    setPhraseSelectionActive(false);
  ...
  const updateProgress = (next: number, readingAnchor: Book['readingAnchor']) => {
    const bounded = Math.max(0, Math.min(1, next));
    setLocalProgress(bounded);
    if (progressTimer.current) window.clearTimeout(progressTimer.current);
    progressTimer.current = window.setTimeout(() => persistProgress({ readingChapter: chapterIndex, readingProgress: bounded, readingAnchor, lastReadAt: Date.now() }), 150);
  };
  ...
  const updatePositionProgress = (nextPage: number, pageCount: number, targetChapter = chapterIndex) => {
    ...
    updateProgress(progress, readingAnchorForPage(pages[boundedPage] ?? [], targetChapter));
  };
  ...
  const changeChapter = (next: number, entry: 'start' | 'end' = 'start') => {
    ...
    setPageIndex(0);
    persistProgress({ readingChapter: bounded, readingProgress: Math.min(1, bounded / Math.max(1, chapters.length)), readingAnchor: undefined, lastReadAt: Date.now() });
  };
  ...
  const beginPhraseSelection = () => {
    selectionRequestSequence.current += 1;
    setSelectionAction(undefined);
    setPhraseSelectionStart(undefined);
    setPhraseSelectionRange(undefined);
    setPhraseSelectionActive(true);
    window.getSelection()?.removeAllRanges();
  };
  const endPhraseSelection = () => {
    selectionRequestSequence.current += 1;
    setSelectionAction(undefined);
    setPhraseSelectionStart(undefined);
    setPhraseSelectionRange(undefined);
  ...
  const startPhraseSelectionMode = () => {
    if (contentMode === 'translation') changeContentMode('original');
    beginPhraseSelection();
    setChromeVisible(false);
  };
  ...
  const paginationOptions = useMemo(() => {
    ...
    return {
      blockCapacity: Math.max(120, Math.floor(glyphsPerLine * lineCount)),
      blockGapWeight: Math.max(9, Math.ceil(glyphsPerLine * (contentMode === 'bilingual' ? 1.08 : 0.75))),
      contentMode,
    };
  }, [contentMode, paginationViewport, readerPreferences.fontSize, readerPreferences.lineHeight]);
  const fallbackPages = useMemo(() => paginateReaderParagraphs(paragraphs, paginationOptions), [paragraphs, paginationOptions]);
  const layoutBlocks = useMemo(() => createReaderPageBlocks(paragraphs, {
    ...paginationOptions,
    blockCapacity: Math.max(64, Math.floor(paginationOptions.blockCapacity / 5)),
  }), [paragraphs, paginationOptions]);
  const paginationKey = `${readerIdentity}:${contentMode}:${readerPreferences.fontFamily}:${readerPreferences.fontSize}:${readerPreferences.lineHeight}:${paginationViewport.contentWidth}:${paginationViewport.contentHeight}:${layoutBlocks.map((block) => `${block.id}:${block.original.length}:${block.translation?.length ?? 0}`).join('|')}`;
  const pages = measuredPagination?.key === paginationKey ? measuredPagination.pages : fallbackPages;
  const pageCount = pages.length;
  const currentPage = pages[Math.min(pageIndex, pageCount - 1)] ?? [];
  const currentBookmarkAnchor = readingAnchorForPage(currentPage, chapterIndex);
  ...
  useLayoutEffect(() => {
    ...
    if (!measureHost || layoutBlocks.length === 0) return;
    const elements = [...measureHost.querySelectorAll<HTMLElement>('[data-reader-measure-block]')];
    if (elements.length !== layoutBlocks.length) return;
    const metrics = elements.map((element) => {
      const style = window.getComputedStyle(element);
      return {
        id: element.dataset.readerMeasureBlock ?? '',
        height: element.getBoundingClientRect().height,
        gapAfter: Number.parseFloat(style.marginBottom) || 0,
      };
    });
    if (metrics.some((metric) => !metric.id || metric.height <= 0)) return;
    const nextPages = paginateMeasuredReaderBlocks(layoutBlocks, metrics, paginationViewport.contentHeight);
    setMeasuredPagination((current) => current?.key === paginationKey && samePageLayout(current.pages, nextPages) ? current : { key: paginationKey, pages: nextPages });
  }, [layoutBlocks, paginationKey, paginationViewport.contentHeight]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const savedAnchor = book.readingAnchor?.chapter === chapterIndex ? book.readingAnchor : undefined;
    const anchorPage = savedAnchor ? pageForReadingAnchor(pages, savedAnchor.paragraphId, savedAnchor.sourceOffset) : -1;
    setPageIndex(anchorPage >= 0 ? anchorPage : pageForReadingProgress(book.readingProgress, chapterIndex, chapters.length, pageCount));
  }, [book.readingAnchor, book.readingProgress, chapterIndex, chapters.length, initialChapter, pageCount, pages]);

  useEffect(() => {
    if (!pendingSearchAnchor || pendingSearchAnchor.chapter !== chapterIndex) return;
    const destinationPage = pageForReadingAnchor(pages, pendingSearchAnchor.paragraphId, pendingSearchAnchor.sourceOffset);
    if (destinationPage < 0) return;
    shouldRestorePagePosition.current = false;
    setPageIndex(destinationPage);
    updatePositionProgress(destinationPage, pageCount, pendingSearchAnchor.chapter);
    setPendingSearchAnchor(undefined);
  }, [chapterIndex, pageCount, pages, pendingSearchAnchor]);
  ...
  const changeContentMode = (mode: ReaderContentMode) => {
    stopSpeech();
    if (mode === 'translation') endPhraseSelection();
    shouldRestorePagePosition.current = false;
    setContentMode(mode);
    setPageIndex(0);
  ...
  const navigateToAnchor = (anchor: BookReadingAnchor) => {
    setContentsOpen(false);
    setChromeVisible(false);
    if (anchor.chapter !== chapterIndex) {
      setPendingSearchAnchor(anchor);
      changeChapter(anchor.chapter);
      return;
    }
    const destinationPage = pageForReadingAnchor(pages, anchor.paragraphId, anchor.sourceOffset);
    if (destinationPage < 0) return;
    shouldRestorePagePosition.current = false;
    setPageIndex(destinationPage);
    updatePositionProgress(destinationPage, pageCount, anchor.chapter);
  };
  const resolveBookmarkAnchor = (): BookReadingAnchor | undefined => {
    if (readerPreferences.readingMode === 'paged') return readingAnchorForPage(currentPage, chapterIndex);
    const canvas = readerCanvasRef.current;
    const canvasTop = canvas?.getBoundingClientRect().top ?? 0;
    const currentBlock = canvas ? [...canvas.querySelectorAll<HTMLElement>('[data-reader-paragraph]')]
      .find((element) => element.getBoundingClientRect().bottom > canvasTop + 72) : undefined;
    const paragraphId = currentBlock?.dataset.readerParagraph;
    if (!paragraphId) return readingAnchorForPage(currentPage, chapterIndex);
    return { chapter: chapterIndex, paragraphId, sourceOffset: Number(currentBlock.dataset.readerSourceOffset ?? 0) };
  };
  const toggleBookmark = async () => {
    const anchor = resolveBookmarkAnchor();
    if (!anchor) {
      setProgressError('当前页面没有可添加书签的位置');
      return;
    }
    const id = bookmarkIdForAnchor(anchor);
    const alreadyBookmarked = bookmarks.some((bookmark) => bookmark.id === id);
    const nextBookmarks = alreadyBookmarked
      ? bookmarks.filter((bookmark) => bookmark.id !== id)
      : [{ ...anchor, id, createdAt: Date.now() }, ...bookmarks].slice(0, 80);
    setBookmarks(nextBookmarks);
    try {
      await onBookmarksChange?.(nextBookmarks);
    } catch {
      setBookmarks(book.bookmarks ?? []);
      setProgressError('保存书签失败');
    }
  };
  const handleReaderTouchStart = (event: TouchEvent<HTMLElement>) => {
    if (phraseSelectionActive) {
  ...
  const handleReaderClick = (event: MouseEvent<HTMLElement>) => {
    ...
    if (phraseSelectionActive) {
      if (!target.closest('.reader-phrase-token')) endPhraseSelection();
      return;
    }
  ...
  const handleReaderKeyDown = (event: KeyboardEvent<HTMLElement>) => {
    if (phraseSelectionActive) return;
  ...
  const allBlocks = paragraphs.map((paragraph) => ({ id: paragraph.id, paragraphId: paragraph.id, original: paragraph.original, translation: paragraph.translation, sourceStart: 0, sourceEnd: paragraph.original.length, isFinalFragment: true }));
  const phraseSelectionBlocks = readerPreferences.readingMode === 'paged' ? currentPage : allBlocks;
  const resolvedPhraseSelection = useMemo(() => (
    phraseSelectionRange ? resolvePhraseSelection(phraseSelectionBlocks, phraseSelectionRange) : undefined
  ), [phraseSelectionBlocks, phraseSelectionRange]);
  const selectedPhraseTokenKeys = resolvedPhraseSelection?.selectedTokenKeys;
  ...
  const handlePhraseTokenClick = (point: PhraseSelectionPoint, event: MouseEvent<HTMLButtonElement>) => {
    ...
    if (!phraseSelectionStart) {
      selectionRequestSequence.current += 1;
      setSelectionAction(undefined);
      setPhraseSelectionRange(undefined);
      setPhraseSelectionStart(point);
      return;
    }
    const range = { start: phraseSelectionStart, end: point };
    const resolved = resolvePhraseSelection(phraseSelectionBlocks, range);
    if (!resolved) {
      setPhraseSelectionStart(point);
      setPhraseSelectionRange(undefined);
      return;
  ...
  const renderOriginalText = (block: ReaderPageBlock) => {
    if (!phraseSelectionActive) return block.original;
    return phraseTextSegments(block.original).map((segment, segmentIndex) => {
      if (!segment.selectable) return <span key={`${block.id}:${segmentIndex}`}>{segment.text}</span>;
      const point = { blockId: block.id, segmentIndex };
      const pointKey = phraseSelectionPointKey(point);
      const className = [
        'reader-phrase-token',
  ...
  const renderBlocks = (blocks: ReaderPageBlock[]) => blocks.map((block) => (
    <div className={`reader-paragraph ${block.isFinalFragment ? '' : 'reader-paragraph--fragment'} ${speechParagraphId === block.paragraphId ? 'reader-paragraph--speaking' : ''}`} key={block.id} data-reader-paragraph={block.paragraphId} data-reader-source-offset={block.sourceStart} aria-current={speechParagraphId === block.paragraphId ? 'true' : undefined}>
      {contentMode !== 'translation' && <p className="reader-original" data-paragraph-id={block.paragraphId}>{renderOriginalText(block)}</p>}
  ...
  return <section className={`reader-page reader-page--${readerPreferences.readingMode} reader-page--theme-${readerPreferences.theme} reader-page--font-${readerPreferences.fontFamily} reader-page--font-${readerPreferences.fontSize} reader-page--line-${readerPreferences.lineHeight} ${phraseSelectionActive ? 'reader-page--phrase-selection' : ''} ${chromeVisible ? 'reader-page--chrome-visible' : ''} ${speechState !== 'idle' ? 'reader-page--speech-active' : ''}`} aria-labelledby="reader-title">
    <ReaderToolbar title={book.title} chapterTitle={chapter?.title || '暂无章节'} chapterIndex={chapterIndex} chapterCount={chapters.length} bookmarked={currentPageBookmarked} onBack={onBack} onToggleBookmark={() => { void toggleBookmark(); }} />
    ...
    <div className="reader-page-measure" aria-hidden="true" style={{ width: paginationViewport.contentWidth }}>
      <div className="reader-page-measure__content" ref={readerPageMeasureRef}>
        {layoutBlocks.map((block) => <div className={`reader-paragraph ${block.isFinalFragment ? '' : 'reader-paragraph--fragment'}`} data-reader-measure-block={block.id} key={block.id}>
          {contentMode !== 'translation' && <p className="reader-original" data-reader-measure-text={block.original} />}
          {contentMode !== 'original' && (block.translation
            ? <p className="reader-translation" lang={bookTargetLanguage} data-reader-measure-text={block.translation} />
            : <p className="reader-translation reader-translation--pending" data-reader-measure-text="本段暂无译文" />)}
        </div>)}
      </div>
    </div>
    ...
    <div className="reader-dock" aria-label="阅读控制">
      <div className="reader-dock__actions">
        ...
        <button type="button" className={`reader-dock__action ${phraseSelectionActive ? 'is-active' : ''}`} aria-pressed={phraseSelectionActive} onClick={() => {
          if (phraseSelectionActive) {
            endPhraseSelection();
            setChromeVisible(true);
          } else {
            startPhraseSelectionMode();
          }
        }} aria-label={phraseSelectionActive ? '退出短语取词' : '开启短语取词'}><TextSelect size={19} /></button>
