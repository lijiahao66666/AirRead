...
import { ArrowLeft, BookOpenText, Check, Download, KeyRound, Pencil, Plus, Save, Trash2, Volume2, X } from 'lucide-react';
...
export function LearningSettingsPage({ store, dailyMinutes, selectedModelId, storyProfile, storyMemory, packs, onMinutesChange, onModelChange, onStoryProfileChange, onChapterWordCountChange, onStartNewStory, generating = false, generationError }: LearningSettingsPageProps) {
  ...
  return <section className="learning-page" aria-labelledby="learning-settings-title">
    <header className="learning-page__header"><div className="learning-page__header-main"><a className="learning-page__back-link" href="#today"><ArrowLeft size={16} /> 今日学习</a><p className="eyebrow">只保留必要设置</p><h2 id="learning-settings-title">学习设置</h2><p className="page-lede">学习目标固定。这里调整每日时间、模型和原创连载存档。</p></div></header>
