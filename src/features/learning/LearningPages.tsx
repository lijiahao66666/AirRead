...
import { AudioLines, ArrowLeft, ArrowRight, BookOpenText, Check, ChevronRight, CircleCheck, Clock3, Headphones, Languages, ListChecks, PenLine, RotateCcw, Volume2 } from 'lucide-react';

import { PwaInstallPrompt } from '../../pwa/PwaInstallPrompt';
import { normalizeExerciseAnswer } from '../../domain/learning/taskExercises';
import type { LearningPack, LearningReviewCard, LearningTask, LearningTaskExercise } from '../../domain/learning/learningTypes';
...
export type TodayPageProps = {
  ...
  prefetchingNextChapter?: boolean;
...
export function TodayPage({ pack, dueReviewCards, completedTaskIds, taskResponses, completedPackIds, generating = false, generationError, nextChapterReady = false, prefetchingNextChapter = false, prefetchError, onGenerate, onGenerateNextChapter, onReview, onSaveTaskResponse, onCompleteTask, onCompletePack }: TodayPageProps) {
  ...
  return <section className="learning-page learning-page--today" aria-labelledby="today-title">
    ...
    <section className={`learning-complete-card${complete ? ' learning-complete-card--done' : ''}`}><CircleCheck size={23} /><div><strong>{complete ? '今天的学习已完成' : readyToComplete ? '可以完成今天的学习了' : '完成训练和复习后再收尾'}</strong><p>{complete ? nextChapterReady ? '下一章已在后台准备好，点击后立即开始。' : prefetchingNextChapter ? '下一章正在后台准备，完成后可直接继续。' : prefetchError ?? '词块已经加入后续复习队列。你可以继续生成下一章，也可以明天再接着学。' : pendingReviewCount > 0 ? `还有 ${pendingReviewCount} 项复习需要完成。` : `还有 ${Math.max(0, pack.tasks.length - completedCount)} 项训练需要完成。`}</p></div>{complete ? <div className="learning-complete-card__actions"><button className="secondary-button" type="button" onClick={onCompletePack}>已完成</button><button className="primary-action" type="button" onClick={onGenerateNextChapter} disabled={generating || prefetchingNextChapter}>{nextChapterReady ? '开始下一章' : prefetchingNextChapter ? '正在准备下一章…' : generating ? '正在创作下一章…' : '继续下一章'}</button></div> : <button className="primary-action" type="button" onClick={onCompletePack} disabled={!readyToComplete}>完成今日学习</button>}</section>
  </section>;
}
